...
@section('header', 'Edit Navbar Link')
...
<div class="max-w-3xl mx-auto">
    <div class="card-premium p-8">
        <div class="mb-8">
            <h3 class="text-xl font-bold text-slate-800">Edit Navbar Link</h3>
            <p class="text-sm text-slate-500">Update the configuration for this navigation link</p>
        </div>

        <form action="{{ route('admin.navbar.update', $item->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="order_column" value="{{ $item->order_column }}" 
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex-grow">
                        <input type="radio" name="position" value="top" class="hidden peer" {{ $item->position == 'top' ? 'checked' : '' }}>
                    ...
                    <label class="flex-grow">
                        <input type="radio" name="position" value="dropdown" class="hidden peer" {{ $item->position == 'dropdown' ? 'checked' : '' }}>
                        <div class="selection-card text-center p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 cursor-pointer transition-all font-bold text-sm">
            ...
            <div>
                ...
                <div class="flex gap-4">
                    <label class="flex-grow">
                        <input type="radio" name="is_active" value="1" class="hidden peer" {{ $item->is_active ? 'checked' : '' }}>
                        <div class="selection-card text-center p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 cursor-pointer transition-all font-bold text-sm">
                            <i class="bi bi-check-circle-fill mr-1"></i> Active
                        </div>
                    </label>
                    <label class="flex-grow">
                        <input type="radio" name="is_active" value="0" class="hidden peer" {{ !$item->is_active ? 'checked' : '' }}>
                        <div class="selection-card text-center p-4 rounded-2xl border-2 border-slate-100 bg-slate-50 cursor-pointer transition-all font-bold text-sm">
