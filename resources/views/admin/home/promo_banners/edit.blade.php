...
<div class="max-w-4xl mx-auto pb-24">
    <div class="card-premium p-8">
        <div class="mb-8">
            <h3 class="text-xl font-bold text-slate-800">Edit Promo Banner</h3>
            <p class="text-sm text-slate-500">Modify seasonal offers and deal sections</p>
        </div>

        <form action="{{ route('admin.home_promo_banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <div class="relative group border-2 border-dashed border-slate-200 rounded-2xl p-4 bg-slate-50 hover:bg-white hover:border-indigo-300 transition-all cursor-pointer overflow-hidden">
                            <input type="file" name="image" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-50" data-image-preview="promo-preview-edit">
                            <div id="promo-preview-container" class="hidden absolute inset-0 w-full h-full bg-white z-40">
                                <img id="promo-preview-edit" src="" class="w-full h-full object-cover">
                            ...
                            <div class="text-center py-4 pointer-events-none">
                                ...
                                <p class="text-xs text-slate-500 mt-2 font-bold uppercase tracking-wider">Click to replace image</p>
                            </div>
                        </div>
