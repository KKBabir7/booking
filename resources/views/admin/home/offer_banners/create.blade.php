...
@section('header', 'Add Offer Banner')
...
<div class="card-premium p-8 max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-8 border-b border-slate-100 pb-5">
        <div>
            <h3 class="text-2xl font-bold text-slate-800">New Offer Banner</h3>
            <p class="text-sm text-slate-500 mt-1">Upload a grid image for the home page</p>
        </div>
    ...
    <form action="{{ route('admin.home_offer_banners.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:bg-slate-50 transition relative overflow-hidden group min-h-[250px]" id="dropZone">
                        <div class="space-y-2 text-center relative z-10" id="uploadPrompt">
                            ...
                            <div class="flex text-sm text-slate-600 justify-center">
                                <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500 px-2 py-1">
                                    ...
                                    <input id="file-upload" name="image" type="file" class="sr-only" accept="image/*" required onchange="previewImage(event)">
                                </label>
                            </div>
                            <p class="text-xs text-slate-500">PNG, JPG, WEBP up to 2MB</p>
                        </div>
                        <img id="imagePreview" class="absolute inset-0 w-full h-full object-cover hidden rounded-xl z-20" />
                        <button type="button" id="removeImageBtn" class="absolute top-2 right-2 bg-rose-500 text-white rounded-full w-8 h-8 flex items-center justify-center hidden z-30 hover:bg-rose-600 shadow-md" onclick="removeImage()">
            ...
            <div class="space-y-6 bg-slate-50/50 p-6 rounded-xl border border-slate-100">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="link" class="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm" placeholder="e.g. /rooms/executive-suite">
                    </div>
                </div>
        ...
        <div class="flex justify-end pt-6 border-t border-slate-100 gap-4">
            ...
            <button type="submit" class="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 transition shadow-md hover:shadow-lg flex items-center gap-2">
                <i class="bi bi-save"></i> Save Offer Banner
            </button>
        </div>
    </form>
</div>
