...
<div class="card-premium p-8 max-w-5xl mx-auto">
    <div class="flex justify-between items-center mb-10 border-b border-slate-100 pb-6">
        <div>
            <h3 class="text-2xl font-bold text-slate-800 tracking-tight">Privacy Policy Settings</h3>
            <p class="text-sm text-slate-500 mt-1">Manage the content and hero section of the privacy policy page.</p>
        </div>
    </div>

    <!-- Hero Section -->
    <form action="{{ route('admin.page.privacy.update') }}" method="POST" enctype="multipart/form-data" class="mb-12">
        @csrf @method('PUT')
        <div class="bg-indigo-50/30 border border-indigo-100 p-8 rounded-3xl relative shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="hero_title" value="{{ $settings['hero_title'] ?? 'Privacy Policy' }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 font-bold">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Tagline / Subtitle</label>
                        <input type="text" name="hero_tagline" value="{{ $settings['hero_tagline'] ?? 'Your Privacy Matters' }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500">
                    </div>
                </div>
                <div>
            ...
            <div class="mt-6 pt-6 border-t border-indigo-100 flex justify-end">
                <button type="submit" class="px-8 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition shadow-lg flex items-center gap-2">
                    <i class="bi bi-shield-check"></i> Save Hero Content
                </button>
            </div>
        </div>
    </form>
    ...
    <form action="{{ route('admin.page.privacy.update') }}" method="POST">
        ...
        <div class="bg-slate-50/50 border border-slate-200 p-8 rounded-3xl relative shadow-sm">
            ...
            <div id="sections-container" class="space-y-6 pt-6">
                @php
                    $sections = isset($settings['content_sections']) ? json_decode($settings['content_sections'], true) : [
                        ['title' => '1. Information We Collect', 'content' => 'Nice Guest House collects information to provide better services...']
                    ];
                @endphp

                @foreach($sections as $index => $section)
                <div class="section-item bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative group transition-all hover:border-indigo-300">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            ...
                            <input type="text" name="content_sections[{{$index}}][title]" value="{{ $section['title'] ?? '' }}" placeholder="e.g. 1. Terms of Use" class="w-full px-4 py-2 border border-slate-100 rounded-xl focus:ring-1 focus:ring-indigo-400 font-bold text-slate-700">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-400 uppercase mb-2">Clause Content (Supports HTML)</label>
                            <textarea name="content_sections[{{$index}}][content]" rows="4" class="w-full px-4 py-3 border border-slate-100 rounded-xl focus:ring-1 focus:ring-indigo-400 text-sm text-slate-600">{{ $section['content'] ?? '' }}</textarea>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="mt-8 flex flex-col items-center gap-6">
                <button type="button" id="add-section" class="btn bg-white border-2 border-dashed border-slate-300 text-slate-600 hover:border-indigo-400 hover:text-indigo-600 font-bold py-3 px-12 rounded-2xl transition flex items-center gap-2">
                    <i class="bi bi-plus-circle fs-5"></i> Add New Clause Section
                </button>
                
                <button type="submit" style="padding-left: 10px;padding-right:10px" class="px-16 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-slate-900 transition shadow-2xl hover:shadow-indigo-400/30 flex items-center gap-3 text-xl">
                    <i class="bi bi-check-circle-fill"></i> Save All Policy Sections
                </button>
            </div>
        </div>
    </form>
...
<template id="section-template">
    <div class="section-item bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative group transition-all hover:border-indigo-300">
        ...
        <div class="grid grid-cols-1 gap-4">
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-2">Clause Heading</label>
                <input type="text" name="content_sections[REPLACE_INDEX][title]" placeholder="New Section Heading" class="w-full px-4 py-2 border border-slate-100 rounded-xl focus:ring-1 focus:ring-indigo-400 font-bold text-slate-700">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-400 uppercase mb-2">Clause Content</label>
                <textarea name="content_sections[REPLACE_INDEX][content]" rows="4" placeholder="Enter details here..." class="w-full px-4 py-3 border border-slate-100 rounded-xl focus:ring-1 focus:ring-indigo-400 text-sm text-slate-600"></textarea>
            </div>
        </div>
    </div>
</template>
