...
@section('header', 'Payment Options Settings')

@section('content')
<div class="card-premium p-8 mb-8 max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h3 class="text-xl font-bold text-slate-800">Payment Options Page Settings</h3>
            <p class="text-sm text-slate-500">Customize the banner and content for the Payment Options page.</p>
        </div>
    </div>

    <form action="{{ route('admin.page.update', 'payment_options') }}" method="POST" enctype="multipart/form-data">
        @csrf @method('PUT')

        <div class="mb-6">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Banner Title</label>
            <input type="text" name="banner_title" value="{{ $settings['banner_title'] ?? 'Our Payment Options' }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-300">
        </div>

        <div class="mb-6">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Banner Subtitle</label>
            <input type="text" name="banner_subtitle" value="{{ $settings['banner_subtitle'] ?? 'Choose from our convenient payment methods.' }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-300">
        </div>
